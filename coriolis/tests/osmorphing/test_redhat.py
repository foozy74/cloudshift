...
class BaseRedHatMorphingToolsTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test_write_nic_configs(
        ...
        mock_test_path.side_effect = (
            lambda path: path != "etc/NetworkManager/system-connections")
    ...
    def test_write_nic_configs_nm(
            self, mock_uuid, mock_test_path,
            mock_exec_cmd_chroot, mock_write_file_sudo):
        nics_info = [{'name': 'eth0'}]

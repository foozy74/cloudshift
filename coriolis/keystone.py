...
def create_trust(ctxt):
    ...
    if not trusts_auth_plugin or not ctxt.auth_token:
        LOG.debug(
            "Skipping Keystone trust creation (noauth context or "
            "trustee config is missing)")
        return
...
def delete_trust(ctxt):
    if ctxt.trust_id:
        ...
        if not auth:
            LOG.warn(
                "No trustee auth plugin configured, cannot delete trust")
            ctxt.trust_id = None
            return

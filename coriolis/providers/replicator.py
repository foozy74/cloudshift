...
class Client(object):
    ...
    def __init__(self, ip, port, credentials, ssh_conn_info,
        ...
        try:
            ...
            is_mock = isinstance(event_handler, mock.Mock)
        except ImportError:
            is_mock = False
        if not is_mock and not isinstance(event_handler, events.EventManager):
            event_handler = events.EventManager(event_handler)
    ...
    def raw_disk_uri(self, disk_name):
        quoted_disk_name = (utils.quote_url(disk_name)
                            if isinstance(disk_name, str) else disk_name)
        diskUri = "%s/device/%s" % (self._base_uri, quoted_disk_name)
        return diskUri
    ...
    def get_status(self, device=None, brief=True):
        ...
        if device is not None:
            quoted_device = (utils.quote_url(device)
                             if isinstance(device, str) else device)
            uri = "%s/%s/" % (uri, quoted_device)
        params = {
            "brief": brief,
        }
    ...
    def get_chunks(self, device, skip_zeros=False):
        quoted_device = (utils.quote_url(device)
                         if isinstance(device, str) else device)
        uri = "%s/api/v1/dev/%s/chunks/" % (self._base_uri, quoted_device)
        params = {
    ...
    def get_changes(self, device):
        quoted_device = (utils.quote_url(device)
                         if isinstance(device, str) else device)
        uri = (
            "%s/api/v1/dev/%s/chunks/changes/"
            % (self._base_uri, quoted_device))
        chunks = self._cli.get(
    ...
    def get_disk_checksum(self, device):
        ...
        quoted_device = (utils.quote_url(device)
                         if isinstance(device, str) else device)
        uri = "%s/api/v1/dev/%s/checksum" % (self._base_uri, quoted_device)
...
class Replicator(object):
    ...
    def __init__(self, conn_info, event_manager, volumes_info, replica_state,
        ...
        try:
            from unittest import mock
            is_mock = isinstance(event_manager, mock.Mock)
        except ImportError:
            is_mock = False
        if not is_mock and not isinstance(event_manager, events.EventManager):
            event_manager = events.EventManager(event_manager)
    ...
    def replicate_disks(
        ...
        for volume in source_volumes_info:
            ...
            disk_size = dst_vol.get("size") or dst_vol.get("size_bytes")
            if disk_size:
                filtered_chunks = []
                for chunk in chunks:
                    offset = int(chunk["offset"])

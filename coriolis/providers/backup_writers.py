...
def _check_deserialize_key(key):
    ...
    if isinstance(key, paramiko.PKey):
        ...
        raise exception.CoriolisException(
            ...
            "a paramiko.PKey instance. Got type '%s'." % (
                type(key)))
...
class HTTPBackupWriterBootstrapper(object):
    ...
    def __init__(self, ssh_conn_info, writer_port):
        ...
        self._set_writer_cmd()

    def _set_writer_cmd(self):
        for path in ["/usr/bin", "/opt"]:
            try:
                out = utils.exec_ssh_cmd(
                    self._ssh,
                    "sudo test -w %s && echo yes || echo no" % path).strip()
                if out == "yes":
                    self._writer_cmd = os.path.join(
                        path, _CORIOLIS_HTTP_WRITER_CMD)
                    break
            except Exception:
                pass

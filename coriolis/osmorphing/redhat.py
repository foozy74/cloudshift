...
class BaseRedHatMorphingTools(base.BaseLinuxOSMorphingTools):
    ...
    def _write_nic_configs(self, nics_info):
        for idx, _ in enumerate(nics_info or []):
            ...
            if not self._test_path(scripts_dir):
                try:
                    self._exec_cmd_chroot(
                        "mkdir -p /etc/sysconfig/network-scripts")
                except Exception as e:
                    LOG.warning(
                        "Failed to create network-scripts directory: %s", e)

            cfg_path = os.path.join(scripts_dir, "ifcfg-%s" % dev_name)
            ...
            try:
                ...
                LOG.warning(
                    "Failed to write legacy ifcfg config for %s: %s",
                    dev_name, e)

            # Check if NetworkManager connections directory exists
            nm_dir = "etc/NetworkManager/system-connections"
            if self._test_path(nm_dir):
                nm_path = os.path.join(nm_dir, "%s.nmconnection" % dev_name)
                try:
                    nm_uuid = str(uuid.uuid4())
                    self._write_file_sudo(
                        nm_path,
                    ...
                    # Set permissions to 600 so NetworkManager will load it
                    self._exec_cmd_chroot("chmod 600 /%s" % nm_path)
                except Exception as e:
                    LOG.warning(
                        "Failed to write NM connection file for %s: %s",
                        dev_name, e)
